...
const ManualBillModal: React.FC<Props> = ({ onClose, onSave, baseCurrency }) => {
  const [vendor, setVendor] = useState('LEGACY VENTURE');
  const [subVendor, setSubVendor] = useState('SAARANGI');
  const [address, setAddress] = useState('27th cross, IV block, Jayanagar');
  const [phone, setPhone] = useState('080 - 490 7000');
  const [gstin, setGstin] = useState('29AAFFL6511N1ZX');
  const [date, setDate] = useState(new Date().toISOString().split('T')[0]);
  const [currency, setCurrency] = useState(baseCurrency);
  const [time, setTime] = useState('21:41');
  const [steward, setSteward] = useState('KRISHNA');
  const [cashier, setCashier] = useState('HEMANT');
  const [billNo, setBillNo] = useState('9022');
  ...
  const handleSave = () => {
    onSave({
      ...
      manualMetadata: {
        ...
        cover: '1',
        session: 'Dinner',
        website: 'www.theroyalcomfort.com',
        gstin
      }
    });
    onClose();
  ...
  // Reusable input class to ensure consistency across all fields
    ...
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[70] flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden max-h-[90vh] flex flex-col border border-slate-100 animate-in fade-in zoom-in duration-200">
        ...
        <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
          <h2 className="text-xs font-black text-slate-900 uppercase tracking-widest">Generate Manual Excel Bill</h2>
        ...
        <div className="p-6 overflow-y-auto space-y-8 scrollbar-hide">
          ...
          <section className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                ...
                <input type="text" value={billNo} onChange={e => setBillNo(e.target.value)} className={inputClasses} placeholder="9000" />
              </div>
              <div>
              ...
              <div>
                ...
                <input type="text" value={time} onChange={e => setTime(e.target.value)} className={inputClasses} placeholder="HH:MM" />
              </div>
              <div>
              ...
              <div>
                ...
                <input type="text" value={steward} onChange={e => setSteward(e.target.value)} className={inputClasses} placeholder="Staff name" />
              </div>
              <div>
                <label className="block text-[9px] font-black text-slate-400 uppercase mb-1.5 tracking-widest">Cashier</label>
                <input type="text" value={cashier} onChange={e => setCashier(e.target.value)} className={inputClasses} placeholder="Cashier name" />
              </div>
            </div>
          </section>
          ...
          <section className="space-y-4">
            ...
            <div className="space-y-3">
              {items.map((item, idx) => (
                <div key={idx} className="flex gap-3 items-center group animate-in slide-in-from-right-2 duration-300" style={{ animationDelay: `${idx * 40}ms` }}>
        ...
        <div className="p-6 bg-slate-50 border-t border-slate-200 flex flex-col md:flex-row gap-4">
          <button 
            onClick={onClose} 
            className="flex-1 bg-white border border-slate-200 text-slate-500 font-black text-[10px] uppercase tracking-widest py-4 rounded-2xl hover:bg-slate-100 transition-all active:scale-95 shadow-sm"
          >
            Discard
          </button>
          <button 
            onClick={handleSave} 
            className="flex-1 bg-indigo-600 text-white font-black text-[10px] uppercase tracking-widest py-4 rounded-2xl shadow-xl shadow-indigo-200 hover:bg-indigo-700 transition-all active:scale-95 flex items-center justify-center gap-2"
          >
